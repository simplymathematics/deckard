...
class Generator(BaseHashable):
    ...
    def parse_prototype(self, filename:Union[str,Path], vars_dict:dict = None) -> dict:
    ...
    def parse_pipe(self):
        ...
        for layer in layers:
            id_ = layers[layer]
            replace[layer+"_folder"] = str(Path(self.root_folder, layer, id_).relative_to(self.root_folder))
            replace[layer+"_params"] = str(Path("..", self.output, layer, id_ + ".yaml").relative_to(self.output))+f":{layer}_params"
    ...
    def __call__(self, path = None, working_directory = None, sort_by = None):
        ...
        if sort_by is not None:
            pd.DataFrame(queue).sort_values(by=sort_by).to_csv(Path(self.output, path, self.queue_file), index=False)
            pd.DataFrame(queue).sort_values(by=sort_by).to_csv(Path(self.output, path, self.main_file), index=False)
        else:
...
if __name__ == '__main__':
    ...
    stage = "prepare"
    param =  stage + "_params"
    gen = Generator(root_folder = root_path, config_path = config_path, config_regex = conf_file, prototype_file = "dvc.yaml", queue_file = "queue.csv")
    assert len(gen) > 0
