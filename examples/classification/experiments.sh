...
for train_size in ${TRAIN_SIZES[@]}; do
    for n_samples in ${N_SAMPLES[@]}; do
        for n_features in ${N_FEATURES[@]}; do
            ...
            HYDRA_FULL_ERROR=1; python ../../deckard/layers/optimize.py --multirun data.generate.n_features=$n_features data.generate.n_informative=$n_informative data.generate.n_samples=$n_samples data.sample.train_size=$train_size model.init.kernel=linear model.init.C=.0001,.001,.01,.1,1,10,100,1000,10000,10000 +stage=train +queue=model_queue "$@" >| logs/models/linear_features-${n_features}_samples-${n_samples}_train-${train_size}.log
            echo "Linear Kernel Done" >> log.txt
            python ../../deckard/layers/optimize.py --multirun data.generate.n_features=$n_features data.generate.n_informative=$n_informative data.generate.n_samples=$n_samples data.sample.train_size=$train_size model.init.kernel=rbf +model.init.gamma=scale model.init.C=.0001,.001,.01,.1,1,10,100,1000,10000,10000 +stage=train +queue=model_queue "$@" >| logs/models/rbf_features-${n_features}_samples-${n_samples}_train-${train_size}.log
