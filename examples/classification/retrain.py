...
from deckard.layers.compile import (flatten_results, parse_results,
                                    unflatten_results)
...
# Generate Models
best_rbf = subset[subset['model.init.kernel'] == 'rbf'].sort_values(by='accuracy', ascending=False).head(1)
best_poly = subset[subset['model.init.kernel'] == 'poly'].sort_values(by='accuracy', ascending=False).head(1)
best_lin = subset[subset['model.init.kernel'] == 'linear'].sort_values(by='accuracy', ascending=False).head(1)
## RBF
...
## ART Models
art_models = [ScikitlearnSVC(model=rbf_model), ScikitlearnSVC(model=poly_model), ScikitlearnSVC(model=linear_model)]
i = 1
epochs = 1
...
for model in art_models:
    ...
    # Define attack
    atk = ProjectedGradientDescent(estimator=model, eps=1, eps_step=.1, max_iter=10, targeted=False, num_random_init=0, batch_size=10)
    # Retrain model if not already done
    if not Path("best_models", f"{name}.pickle").exists():
        raise ValueError(f"Model best_models/{name}.pickle not found. Please run the model queue first.")
    ...
    if not Path("retrain", name, f"{epochs}.pkl").exists():
        results, outputs = retrain_loop(model, X_train, y_train, X_test, y_test, atk, epochs=epochs, attack_size=100)
    ...
    for folder in tqdm(os.listdir("reports/attack")):
        ...
        if Path("reports/attack", folder, "samples.json").exists():
            ...
            false_confidence = y_test[:len(probs)] - probs[:,1]
            avg_prob = np.mean(false_confidence)
            pd.DataFrame(probs).to_csv(Path("reports/attack", folder, f"probs_after_retraining_{name}.csv"))
            with open(Path("reports/attack", folder, "scores.json"), "r") as f:
                scores = json.load(f)
            if "False Confidence" in scores:
                del scores['False Confidence']
