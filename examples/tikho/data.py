...
class Data(collections.namedtuple(typename = 'Data', field_names = 'name, params, generated', defaults = (None,))):
    ...
    def load(self):
        ...
        # If the data is among the sklearn "generated" datasets
            ...
            assert self.generated is not None, ValueError("generated datasets require the generated parameter")
        ...
        # If the data is a json
            ...
            if "X_train" in data:
                assert hasattr(data, "y_train") and hasattr(data, "X_test") and hasattr(data, "y_test"), ValueError("X_train, y_train, X_test, and y_test must all be present in the pickle file")
                X_train = data.X_train
                y_train = data.y_train
                X_test = data.X_test
                y_test = data.y_test
                return Namespace(X_train = X_train, y_train = y_train, X_test = X_test, y_test = y_test)
            else:
                assert hasattr(data, "X") and hasattr(data, "y"), ValueError("data must have X and y attributes")
                big_X = data.X
                big_y = data.y
        ...
        elif isinstance(self.name, Path) and self.name.exists() and str(self.name).endswith(".pkl"):
        ...
        if "time_series" in self.params and self.params["time_series"] == True:
        ...
        # timeseries split
            elif time_series == True:
                assert "test_size"  or "train_size" in self.params, "if time series, test_size must be specified"
                max_train_size = self.params.pop("train_size") if "train_size" in self.params else int(round(len(big_X) * .8))
                gap = 0 if "gap" not in self.params else self.params.pop("gap")
                test_size = self.params.pop("test_size") if "test_size" in self.params else int(round(len(big_X) / (2 + 1 + gap)))
                splitter = TimeSeriesSplit(n_splits = 2, max_train_size = max_train_size, test_size = test_size, gap = gap)
                initial = 0
                assert initial < len(big_X), ValueError("random_state is used to select the index of the of a subset of time series data and must be less than the length of said data + test_size")
    ...
    def visualize(self, data:Namespace, files:dict, plots:dict):
        ...
        paths = []
        if len(plots.keys()) > 0:
            assert "path" in files, "Path to save plots is not specified in the files entry in params.yaml"
...
if '__main__' == __name__:
    ...
    data_document_tag = u"""!Data:""" + data_document
