...
class HTMLHandler(watchdog.events.PatternMatchingEventHandler):
    def __init__(self, pattern="**/*predictions.json"):
    ...
    def on_closed(self, event):
        ...
        print(
            f'{progress["achieved"]*100:.2f} percent of experiments completed!'.format(
    ...
    def merge_all_results(
        ...
        big["link"] = [
            Path(big.index[i], big.index[i], "report.html") for i in range(len(big))
        ]
        print(big["link"])
        big["link"] = big["link"].apply(
            lambda x: '<a href="{}">{}</a>'.format(x, "Summary"),
        )
        big.index = big["link"]
        del big["link"]
        big["predictions"] = pred_paths
        big["predictions"] = big["predictions"].apply(
            lambda x: '<a href="{}">{}</a>'.format(x, "Predictions"),
        )
        return big
