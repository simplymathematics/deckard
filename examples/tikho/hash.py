...
if __name__ == "__main__":
    ...
    ground_truth = Path(
        params["data"]["files"]["path"],
        params["data"]["files"]["ground_truth"],
    )
    predictions = Path(
        params["model"]["files"]["path"],
        params["model"]["files"]["predictions"],
    )
    scores = Path(
        params["model"]["files"]["path"],
        params["model"]["metrics"]["scores"],
    )
    ...
    assert Path(
        new_path / filename,
    ).exists(), f"Params was not saved: {new_path / filename}"
