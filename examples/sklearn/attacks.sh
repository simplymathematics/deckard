...
# FGM
bash models.sh $@ ++attack.init.name=art.attacks.evasion.FastGradientMethod ++attack.init.norm=1,2,inf ++attack.init.eps=.001,.01,.1,.3,.5,1 ++attack.init.eps_step=.001,.01,.1,.3,.5,1 ++attack.init.batch_size=100

# PGD
bash models.sh $@ ++attack.init.name=art.attacks.evasion.ProjectedGradientDescent ++attack.init.norm=1,2,inf ++attack.init.eps_step=.001,.01,.1,.3,.5,1 $@ ++attack.init.batch_size=100
...
#Hop Skip Jump
bash models.sh $@ ++attack.init.name=art.attacks.evasion.HopSkipJump ++attack.init.batch_size=100 ++attack.init.batch_size=100
