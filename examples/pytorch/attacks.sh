...
bash models.sh ++attack.init.name=art.attacks.evasion.FastGradientMethod +attack.init.eps=.01,.03,.3,.1 +attack.init.norm=inf,1,2 +attack.init.eps_step=.001,.003,.01 +attack.init.batch_size=1024 ++stage=attack  ++hydra.sweeper.study_name=attack ++hydra.sweeper.storage=sqlite:///attack.db ++hydra.sweeper.direction=minimize $@ --multirun
