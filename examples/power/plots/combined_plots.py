...
for data in datasets:
    df = pd.read_csv(Path(normal_dir) / data / "power.csv", index_col=0, low_memory=False)
    df['dataset'] = data
    ...
    if Path(normal_dir, extra_data_dir, data, "power.csv").exists():
        extra_df = pd.read_csv(Path(normal_dir, extra_data_dir, data, "power.csv"), index_col=0, low_memory=False)
...
adv_pred_samples = pd.Series(big_df['adv_predict_time'] / big_df['adv_predict_time_per_sample'])
...
big_df['predict_time'] = big_df['predict_time'] / (big_df['ben_pred_samples'] * .25)
...
for device in big_df.device_id.unique():
    big_df.loc[big_df.device_id == device, "peak_memory_bandwith"] = float(memory_bandwith[device])
...
adv_acc = sns.boxenplot(data=big_df, x="dataset", y="adv_accuracy", hue="Device", ax=ax[1], )
...
train_time = sns.boxenplot(data=big_df, x="dataset", y="train_time", hue="Device", ax=ax[0],)
train_time.set_title("Average Training Time per Sample")
train_time.set_ylabel("$t_{t}$ (seconds)")
train_time.set_xlabel("Dataset")
predict_time = sns.boxenplot(data=big_df, x="dataset", y="predict_time", hue="Device", ax=ax[1],)
...
adv_fit_time = sns.boxenplot(data=big_df, x="dataset", y="adv_fit_time", hue="Device", ax=ax[2],)
...
train_time = sns.boxenplot(data=big_df, x="dataset", y="train_power", hue="Device", ax=ax[0],)
train_time.set_title("Average Training Power per Sample")
train_time.set_ylabel("$P_{t}$ (Watts)")
train_time.set_xlabel("Dataset")
predict_time = sns.boxenplot(data=big_df, x="dataset", y="predict_power", hue="Device", ax=ax[1],)
predict_time.set_title("Average Inference Power per Sample")
predict_time.set_ylabel("$P_{i}$ (Watts)")
predict_time.set_xlabel("Dataset")
adv_fit_time = sns.boxenplot(data=big_df, x="dataset", y="adv_fit_power", hue="Device", ax=ax[2],)
...
fig, ax = plt.subplots(1, 3, figsize=(18, 5))
train_cost = sns.boxenplot(data=big_df, x="dataset", y="train_cost", hue="Device", ax=ax[0],)
train_cost.set_title("Average Training Cost per Sample")
train_cost.set_ylabel("$C_{t}$ (USD)")
train_cost.set_xlabel("Dataset")
predict_cost = sns.boxenplot(data=big_df, x="dataset", y="predict_cost", hue="Device", ax=ax[1], legend=False)
predict_cost.set_title("Average Inference Cost per Sample")
predict_cost.set_ylabel("$C_{i}$ (USD)")
predict_cost.set_xlabel("Dataset")
adv_fit_cost = sns.boxenplot(data=big_df, x="dataset", y="adv_fit_cost", hue="Device", ax=ax[2],)
