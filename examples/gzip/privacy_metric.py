...
from gzip_classifier import GzipKNN, load_data, GzipSVC
...
def calculate_privacy_risk(X_train, X_test, y_train, y_test, metric):
    ...
    privacy_risk = art.metrics.SHAPr(target_estimator=est, x_train=X_train, x_test=X_test, y_train=y_train, y_test=y_test)
...
def line_search_metrics(X_train, X_test, y_train, y_test, metrics):
    ...
    for metric in metrics:
        scores, privacy_risks = calculate_privacy_risk(X_train, X_test, y_train, y_test, metric)
        metric_dict[metric] = {}
        ...
        for i in range(nb_classes):
            ...
            privacy_risk = .01 if privacy_risk == 0 else privacy_risk
            shaprs.append(privacy_risk)
            accuracy = round(np.mean(scores[idxs]),2)
            accuracies.append(accuracy)
            print(f"{metric.capitalize()} SHAPr for class {i}: {privacy_risk}; Accuracy: {accuracy}")
    ...
    labels = [f"{label_prefixes[i]}_{j}" for i in range(nb_classes) for j in range(len(label_prefixes))]
    values = []
    ...
    for i in range(nb_classes):
        ...
        values.append(sub_values)
    plt.simple_multiple_bar(metrics, values, title="Accuracy and SHAPr for different metrics", labels=labels)
    plt.show()
    return metric_dict

if __name__ == "__main__":
    metrics = ["gzip", "lzma", "bz2", "zstd", "pkl", "levenshtein", "ratio", "hamming", "jaro", "jaro_winkler", "seqratio",]
