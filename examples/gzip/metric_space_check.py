...
def check_loop(number =1000, sig_figs=2, max_size=1000, data='random'):
    ...
    with tqdm(total=number) as pbar:
        for i in range(number):
            ...
            try:
                assert check_symmetry(x,y,z, sig_figs=sig_figs) is None, "Not Symmetric"
            except:
                symmetric_failures += 1
            try:
                assert check_triangle_inequality(x,y,z) is None, "Triangle Inequality Broken"
            except:
                triangle_failures += 1
            try:
                assert check_zero(x,y,z, sig_figs=sig_figs) is None, "Zero Identity broken"
            except:
                zero_failures += 1
            try:
