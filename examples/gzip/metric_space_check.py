...
def check_triangle_inequality(x, y, z):
    ...
    xy = ncd(x, y)
    if xz > xy + yz:
        raise ValueError(f"Triangle Inequality failed for {x}, {y}, {z}. <x,z> = {xz} > <x,y> + <y,z> = {xy + yz}")
    return None
...
def check_loop(samples=1000, sig_figs=2, max_string_size=1000, data="random", alphabet_size=52):
    triangle_failures = 0
    ...
    if data == "random":
        ...
        combinations = find_all_combinations(max_alphabet_size=alphabet_size, max_string_size=max_string_size, reverse=True)
        total = len(combinations)
    else:
        raise NotImplementedError("Only random strings and alphabet combinations are supported at the data.")
    with tqdm(total=total) as pbar:
        for i in range(total):
            if data == "random":
                x = string_generator(size=random.randint(1, max_string_size), alphabet_size=alphabet_size)
                y = string_generator(size=random.randint(1, max_string_size), alphabet_size=alphabet_size)
                z = string_generator(size=random.randint(1, max_string_size), alphabet_size=alphabet_size)
            elif data == "combinations":
...
def check_all_metric_space_assumptions(max_sig_figs=10, samples=1000, max_string_size=1000, max_alphabet_size=95, iterate="sig_figs", data = "random"):
    symmetries = []
    zeroes = []
    triangles = []
    positivities = []
    if iterate == "sig_figs":
        kwargs = {"samples": samples, "max_string_size": max_string_size, "alphabet_size": max_alphabet_size, "data": data}
        # Run all sig figs
        iterator = range(0, max_sig_figs)
        title = "Significant Figures"
    elif iterate == "max_string_size":
        kwargs = {"sig_figs": max_sig_figs, "samples": samples, "alphabet_size": max_alphabet_size, "data": data}
        ...
        kwargs = {"sig_figs": max_sig_figs, "samples": samples, "max_string_size": max_string_size, "data": data}
    ...
    plt.xticks(range(len(iterator)), iterator)  # Set the x-axis ticks to be the iterator
...
if __name__ == "__main__":
    ...
    parser.add_argument("--iterate", type=str, nargs="+", default=["sig_figs", "max_string_size", "alphabet_size"])
    ...
    for iterate in args.iterate:
        check_all_metric_space_assumptions(max_sig_figs=args.sig_figs, samples=1000, max_string_size=args.max_string_size, max_alphabet_size=args.max_alphabet_size, iterate=iterate, data=args.data)
