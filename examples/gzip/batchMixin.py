...
test_scores = []
class BatchedMixin:
    def __init__(self, batch_size:int=10, max_batches: int=100, nb_epoch=1, **kwargs):
        ...
        if "m" in kwargs:
            logger.warning(f"Parameter 'm' is being overwritten with batch_size={self.batch_size}.")
            kwargs['m'] = self.batch_size
        super().__init__(**kwargs)
        self.predict = self.batched_predict(self.predict)
        if hasattr(self, "_find_best_samples"):
            self._find_best_samples = self.batched_find_best_samples(self._find_best_samples)
    ...
    def batched_fit(self, fit_func):
        def wrapper(*args, **kwargs):
            ...
            if n_batches > self.max_batches:
                logger.warning(f"Number of batches ({n_batches}) is greater than max_batches ({self.max_batches}). Using max_batches.")
            ...
            for i in tqdm(range(n_batches), desc="Fitting batches", total=n_batches, leave=False, dynamic_ncols=True):
                ...
                print(f"Shape of X_batch is {X_batch.shape} and shape of y_batch is {y_batch.shape}")
                fit_func(X_batch, y_batch, **kwargs)
                if self.nb_epoch > 1:
                    continue
                train_score = self.score(X_batch, y_batch)
                test_score = self.score(X_train, y_train)
                print(f"Batch {i+1} of {n_batches} - Train score: {np.mean(train_score)}; Test score: {np.mean(test_score)}")
...
if __name__ == "__main__":
    ...
    big_y = []
    for i in range(100):
        X, y = make_classification(n_samples=100, n_features=20, n_informative=19, n_redundant=1, n_classes=2, random_state=42+i)
