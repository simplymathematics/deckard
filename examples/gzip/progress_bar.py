...
def progress_bar_main(folder, regex, sleep_time, count_files, total_runs):
    while sys.stdin.isatty():
        ...
        bar_string = f"Progress: [{bar}] {completed_runs}/{total_runs} ({progress*100:.2f}%)"
        ...
        if progress > start_progress and completed_runs > first_count:
            ...
            if time_remaining > 7 * 24 * 3600:
                completed_at_string = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(end_time + time_remaining))
                sleep_time = min(3600, sleep_time)
            elif time_remaining > 24 * 3600:
                completed_at_string = time.strftime("%m-%d %H:%M:%S", time.localtime(end_time + time_remaining))
                sleep_time = min(3600, sleep_time)
            else:
                completed_at_string = time.strftime("%H:%M:%S", time.localtime(end_time + time_remaining))
                sleep_time = min(600, sleep_time)
        else:
            completed_at_string = "Unknown"
        # Erase the current line content
        sys.stdout.write('\x1b[2K')
        print(bar_string + f". To be completed at: {completed_at_string}", end="\r", flush=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Monitor progress of DVC pipeline runs")
    parser.add_argument("--folder", default="output/", help="Output folder path")
    parser.add_argument("--regex", default="*/logs/search/*/*/*/optimization_results.yaml", help="File pattern to match")
    parser.add_argument("--stage", default="grid_search", help="DVC pipeline stage name")
    parser.add_argument("--dvc-file", default="dvc.yaml", help="DVC pipeline file path")
    parser.add_argument("--scale", type=int, default=1, help="Scale factor for total runs")
    parser.add_argument("--sleep-time", type=int, default=1, help="Sleep time between checks in seconds")
    parser.add_argument("--total-runs", type=int, default=None, help="Total number of runs (optional)")
