...
def check_triangle_inequality(matrix: np.ndarray, max_samples=-1) -> bool:
    ...
    assert max_samples <= n*n, "Number of samples is greater than the number of elements in the matrix."
    # Sample the indices
    indices = np.random.choice(n, size=(max_samples, 3))
    counts = 0
    for i, j, k in tqdm(indices, desc="Checking triangle inequality", position=1, leave=False):
...
def check_zero_identity(matrix: np.ndarray) -> bool:
    ...
    logger.debug(f"Number of non-zero elements except diagonal: {non_diagonal_failures}")
...
def count_failures(matrix: np.ndarray) -> dict:
    ...
    failures['symmetry'] = failures['symmetry'] / total / 2 # Divide by 2 because we assume symmetry
    failures['non_negative'] = failures['non_negative'] / total # Divide by the total number of elements
...
def plot_results(df: pd.DataFrame, results_plot: Path):
    ...
    df['modified'] 
    algorithm = df['modified'].astype(int) + df['symmetric'].astype(int) # If modified and symmetric, then 2, if not modified and symmetric, then 1, if not modified and not symmetric, then 0. The 4th case isn't meaningful.
    # Vanilla, Assumed Symmetry, Enforced Symmetry
    df['algorithm'] = algorithm.map({0: "Vanilla", 1: "Assumed Symmetry", 2: "Enforced Symmetry"})
    ...
    g = sns.catplot(x="variable", y="value", data=df, hue="compressor", col="dataset", kind="bar", row="algorithm", sharex=True, sharey=True)
    # xlabel
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Check matrices for metric space properties.")
    parser.add_argument("--working_dir", type=str, help="Directory containing the matrices to check.", required=True)
    parser.add_argument("--file_regex", type=str, help="Regex to match the files to check.", required=True)
    parser.add_argument("--results_file", type=str, help="Name of the output file.", default="failures.csv")
    parser.add_argument("--log_file", type=str, help="Name of the log file.", default="check_matrices.log")
    parser.add_argument("--results_plot", type=str, help="Name of the plot file.", default="failures.pdf")
    parser.add_argument("--results_folder", type=str, help="Name of the results folder.", default="check_matrices")
