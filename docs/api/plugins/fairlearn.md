...
deckard framework.
See also: {doc}`../pytorch/index` for torch-backed fairness workflows.
...
- the same stage-aware model contract documented in {doc}`../model/index` applies to
...
See also {doc}`../score/index` for scorer profile composition and {doc}`../attack/index` for
