...
{doc}`../experiment/index` and follows [Hydra](https://hydra.cc) composition patterns.
...
- {doc}`../score/index` for shared scorer profiles and metric composition
- {doc}`../model/index` for defense-enabled model orchestration
- {doc}`../attack/index` for attack execution with survival configurations
- {doc}`../plot/index` for survival curve plotting backends
- {doc}`../data/pipeline` and {doc}`../data/sample` for survival preprocessing/split workflows
...
- `data`
- {doc}`../model/index` (string model name/alias, for example `cox` or `weibull`)
- `target`
- `event_col`
- `duration_col`
...
- {doc}`../data/index` — general data configuration including {mod}`deckard.plugins.lifelines.data`
- {doc}`../model/index` — general model configuration including {mod}`deckard.plugins.lifelines.model`
- {doc}`../score/index` — scoring framework including {mod}`deckard.plugins.lifelines.score`
- {doc}`../experiment/index` — experiment orchestration including
  {class}`~deckard.plugins.lifelines.experiment.SurvivalExperimentConfig`
- {doc}`../plot/index` — visualization including survival curve plotting
- {doc}`../data/pipeline` — data-pipeline composition for survival datasets
- {doc}`../data/sample` — split and fold strategies used by survival runs
- {doc}`../pytorch/index` — optional deep learning survival models
