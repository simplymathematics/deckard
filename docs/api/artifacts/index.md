...
Artifact persistence works alongside {doc}`../file/index` path resolution and the
orchestration behavior documented in {doc}`../experiment/index`.
...
1. Resolve canonical file aliases via {doc}`../file/index`.
...
- {doc}`../file/index` — canonical path and file alias behavior
- {doc}`../experiment/index` — orchestration and persistence timing
- {doc}`../plot/index` — plot backends consuming persisted artifacts
- {doc}`../layers/index` — post-hoc layers reading persisted outputs
