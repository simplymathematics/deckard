...
- `mode`: runtime scoring mode, for example `train`, `test`, `val`, `attack`, or
  ...
  {func}`~deckard.score.normalize_scorer_mode`).
...
- {doc}`../data/index` — data configuration
- {doc}`../model/index` — model configuration and evaluation
- {doc}`../attack/index` — attack scoring
- {doc}`../experiment/index` — experiment orchestration
- {doc}`../plugins/lifelines` — survival-specific metrics
- {doc}`../plugins/anjana` — anonymization metrics
