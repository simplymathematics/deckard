...
- ``plot=confusion_matrix`` / ``plot=discrimination_threshold``
- ``plot=residuals`` / ``plot=prediction_error``
...
Plotting attack/scorer outputs with Seaborn
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Attack, fairness, and custom scorer outputs are best visualized through the
Seaborn backend using the score artifact generated by an experiment run.

Multi-attack runs can include both unsuffixed and alias-suffixed metric keys,
for example ``evasion_accuracy`` and ``evasion_accuracy_hsj``. These can be
plotted directly as separate series.
...
.. code-block:: yaml
   ...
   y: evasion_accuracy
   hue: attack_alias
   title: Evasion Accuracy By Attack
   plot_file: plots/evasion_accuracy_by_attack.png
...
.. code-block:: yaml
   ...
   data_file: outputs/scores.csv
   x: accuracy
   y: evasion_accuracy_hsj
   hue: defense_alias
   title: Benign vs HSJ Accuracy
   plot_file: plots/benign_vs_hsj_accuracy.png
