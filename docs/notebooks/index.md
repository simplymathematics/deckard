...
- [sklearn](sklearn) demonstrates the default tabular pipeline stack, including
  data transforms, model execution, and score composition.
- [pytorch](pytorch) demonstrates torch-native training and evaluation flows,
  including dataset loaders, tensor models, and attack-aware scoring.

Plugin functionality:

- [fairlearn](fairlearn) shows fairness-aware data, model, and group-metric
  workflows.
- {doc}`anjana </notebooks/anjana>` shows anonymization-aware preprocessing and
  privacy-oriented evaluation.
- [lifelines](lifelines) shows survival-analysis and time-to-event workflows.
- [seaborn](seaborn) and [yellowbrick](yellowbrick) show reporting,
  diagnostics, and visualization integrations.
- [art_attacks](art_attacks), [art_defenses](art_defenses), and
...
| --- | --- | --- |
| Files-only persistence aliases | [sklearn](sklearn), [hydra](hydra), [optimize](optimize), [dvc](dvc) | `files={...}` payloads, `+files.params_file=...`, `+files.score_file=...`, and persisted run artifacts |
| Canonical timing keys plus extensibility | [scoring](scoring), [sklearn](sklearn), [art_attacks](art_attacks), [fairlearn](fairlearn) | flat timing keys such as `prediction_time` and richer nested timing payloads for plugin or attack-specific execution |
| Stage and mode normalization plus hook ordering | [scoring](scoring), [hydra](hydra), [optimize](optimize), [anjana](anjana), [fairlearn](fairlearn) | explicit `score_mode`, stage-aware hooks, and plugin hook merge behavior |
| Cache-key determinism and selective invalidation | [optimize](optimize), [dvc](dvc), [artifacts](artifacts) | repeated run or multirun templates, cache reuse expectations, and resolved artifact identity |
| Human-readable YAML and JSON params or score artifacts | [scoring](scoring), [hydra](hydra), [optimize](optimize), [dvc](dvc), [artifacts](artifacts) | `scores.json`, `params.yaml`, DVC-generated manifests, and round-trip artifact inspection |
...
| --- | --- | --- |
| [sklearn](sklearn) | Canonical tabular runtime flow | score artifacts, experiment outputs, and split-aware scoring examples |
| [pytorch](pytorch) | Torch-native training and checkpoint flow | checkpoint files, score artifacts, and model-state examples |
...
| [art_defenses](art_defenses) | Defense execution and evaluation | defended artifact outputs and comparison metrics |
| [detector](detector) | Detector fit or detect orchestration | detector outputs, filtered artifacts, and detector score payloads |
| [fairlearn](fairlearn) | Fairness-aware data/model/score flow | group metric score artifacts and fairness-specific runtime outputs |
| [anjana](anjana) | Privacy-aware preprocessing and privacy scoring | anonymized artifacts, defended outputs, and privacy score files |
| [lifelines](lifelines) | Survival-analysis runtime flow | survival metrics, tables, and backend-specific persisted outputs |
| [seaborn](seaborn) | Results-table driven plotting | figure outputs and plot-spec-backed artifacts |
| [yellowbrick](yellowbrick) | Experiment-backed diagnostics | diagnostic figures and model-analysis outputs |
| [deckard](deckard) | Layer and script walkthroughs | CLI-adjacent params or score examples and generated helper artifacts |
