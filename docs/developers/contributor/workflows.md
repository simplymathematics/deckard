...
- See [Developer Documentation Index](../index) for details

**Docs Cross-Reference Audit (Per-File Tracking):**

- Use repository enforcement to emit a per-file docs audit report with line
  coverage and violation counts:
  - `python scripts/repository_enforcement.py --scope deckard --docs-scope docs --docs-audit-report build/workflow-runs/docs-crosslink-audit.json`
- The JSON report is intended for CI artifacts and local tracking while
  reducing cross-link debt in `docs/*.md` and notebook markdown cells.
