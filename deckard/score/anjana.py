...
def _resolve_frame_and_context(data=None, y_pred=None, **kwargs):
    ...
    if isinstance(y_pred, pd.DataFrame):
        frame = y_pred
    elif data is not None and isinstance(getattr(data, "_X", None), pd.DataFrame):
        frame = data._X
    if frame is None:
        raise ValueError("ANJANA scorers require a pandas.DataFrame via y_pred or data._X")

    quasi_ident = kwargs.get("quasi_ident", getattr(data, "quasi_identifiers", None))
    sens_att = kwargs.get("sens_att", getattr(data, "sensitive_attribute", None))
...
def anjana_k_anonymity_score(y_true=None, y_pred=None, data=None, **kwargs):
    ...
    frame, quasi_ident, _ = _resolve_frame_and_context(data=data, y_pred=y_pred, **kwargs)
...
def anjana_l_diversity_score(y_true=None, y_pred=None, data=None, **kwargs):
    ...
    frame, quasi_ident, sens_att = _resolve_frame_and_context(data=data, y_pred=y_pred, **kwargs)
    if sens_att is None:
        raise ValueError("ANJANA l-diversity scorer requires sens_att/sensitive_attribute")
...
def anjana_t_closeness_score(y_true=None, y_pred=None, data=None, **kwargs):
    ...
    frame, quasi_ident, sens_att = _resolve_frame_and_context(data=data, y_pred=y_pred, **kwargs)
    if sens_att is None:
        raise ValueError("ANJANA t-closeness scorer requires sens_att/sensitive_attribute")
...
safe_store(group="score", name="anjana_model", node=DefaultAnjanaModelScoreConfig)
