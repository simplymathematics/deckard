...
def _resolve_reference_vector(y_true, X: pd.DataFrame, **kwargs):
	...
	if reference_column is not None:
		if reference_column not in X.columns:
			raise ValueError(f"reference_column '{reference_column}' not found in features")
...
def _feature_mutual_information_vector(y_true, y_pred, **kwargs) -> np.ndarray:
	...
	if X.shape[1] == 0:
		raise ValueError("No feature columns available for mutual-information analysis")
...
def data_mutual_information_mean_score(y_true, y_pred, **kwargs):
	mi = _feature_mutual_information_vector(y_true=y_true, y_pred=y_pred, **kwargs)
...
safe_store(group="score", name="data-classification", node=DefaultDataClassificationConfig)
safe_store(group="score", name="data-regression", node=DefaultDataRegressionConfig)
