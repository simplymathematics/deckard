...
model_call_parser = create_parser_from_function(ModelConfig.__call__, exclude=["self", "data"])
defense_call_parser = create_parser_from_function(DefenseConfig.__call__, exclude=["self", "data", "estimator"])
attack_call_parser = create_parser_from_function(AttackConfig.__call__, exclude=["self", "data", "estimator"])
...
class ExperimentConfig(ConfigBase):
    ...
    def set_device(self, device: Union[str, int] = "cpu"):
        ...
        if self.library == "tensorflow":
            ...
            if device == "cpu":
                ...
                logger.warning("Invalid device specified for TensorFlow, using default device.")
        elif self.library == "pytorch":
            import torch
            if device == "cpu":
                torch_device = torch.device("cpu")
                logger.info("Using CPU for PyTorch")
            elif isinstance(device, str) and "gpu" in device.lower() and torch.cuda.is_available():
    ...
    def __post_init__(self):
        ...
        assert isinstance(self.data_config, DataConfig), "data_config must be an instance of DataConfig"
        self.data_config.__post_init__()
        if self.model_config:
            assert isinstance(self.model_config, ModelConfig), "model_config must be an instance of ModelConfig"
            self.model_config.__post_init__()
        if self.attack_config:
            assert isinstance(self.attack_config, AttackConfig), "attack_config must be an instance of AttackConfig"
        ...
        if self.file_config is None:
            ...
            assert isinstance(self.file_config, FileConfig), "file_config must be an instance of FileConfig"
    ...
    def _hash_from_config_list(self, config_list: List[ConfigBase]) -> str:
        ...
        for conf in config_list:
            assert isinstance(conf, ConfigBase), "All items in config_list must be instances of ConfigBase"
            to_string = "".join([str(getattr(conf, attr)) for attr in dir(conf) if not attr.startswith("_") and not callable(getattr(conf, attr))])
    ...
    def __call__(self, **kwargs):
        ...
        assert hasattr(data, "X_train"), "data_config must return an object with X_train attribute"
        assert hasattr(data, "y_train"), "data_config must return an object with y_train attribute"
        assert hasattr(data, "X_test"), "data_config must return an object with X_test attribute"
        assert hasattr(data, "y_test"), "data_config must return an object with y_test attribute"
        ...
        if self.model_config:
            ...
            assert hasattr(model, "training_predictions"), "model must have training_predictions attribute after training"
            assert hasattr(model, "predictions"), "model must have predictions attribute after training"
            assert hasattr(model, "score_dict"), "model must have score_dict attribute after training"
            for k,v in data.score_dict.items():
                assert k in model.score_dict, f"score {k} from data not found in model score_dict"
                assert v == model.score_dict[k], f"score {k} from data does not match model score_dict"
        ...
        if self.attack_config:
            ...
            data, model, attack = self.attack_config(data=data, model=model, **attack_call_params, **file_dict)
            assert hasattr(attack, "attack"), "attack must have attack attribute after training"
            assert hasattr(attack, "attack_training_predictions"), "attack must have attack_training_predictions attribute after training"
            assert hasattr(attack, "attack_predictions"), "attack must have attack_predictions attribute after training"
            assert hasattr(attack, "attack_score_dict"), "attack must have attack_score_dict attribute after training"
            for k,v in model.score_dict.items():
                assert k in attack.attack_score_dict, f"score {k} from model not found in attack score_dict"
                assert v == attack.attack_score_dict[k], f"score {k} from model does not match attack score_dict"
