...
class FlexNet(nn.Module if nn else object):
    ...
    """
    def __init__(self, backbone_name="resnet18", num_channels=3, num_classes=2, pretrained=True):
        if nn is None or models is None:
            raise ImportError("FlexibleNet requires torch and torchvision to be installed.")
        super().__init__()
    ...
    def _adapt_input_channels(self, num_channels):
        ...
        if hasattr(self.backbone, 'conv1'):  # ResNet style
        ...
        elif hasattr(self.backbone, 'features') and isinstance(self.backbone.features[0], nn.Conv2d):  # VGG / ConvNeXt style
            ...
            raise NotImplementedError("Channel adaptation not automatically supported for this backbone architecture.")
    ...
    def _adapt_output_classes(self, num_classes) -> int:
        ...
        elif hasattr(self.backbone, 'classifier') and isinstance(self.backbone.classifier, nn.Sequential):  # VGG style
            in_features = self.backbone.classifier[-1].in_features
            self.backbone.classifier[-1] = nn.Linear(in_features, num_classes)
        elif hasattr(self.backbone, 'classifier') and isinstance(self.backbone.classifier, nn.Linear):  # MobileNet style
            in_features = self.backbone.classifier.in_features
            self.backbone.classifier = nn.Linear(in_features, num_classes)
        elif hasattr(self.backbone, 'heads') and hasattr(self.backbone.heads, 'head'):  # ViT style
            in_features = self.backbone.heads.head.in_features
            self.backbone.heads.head = nn.Linear(in_features, num_classes)
        else:
            raise NotImplementedError("Classification head adaptation not supported for this backbone architecture.")
        return in_features
