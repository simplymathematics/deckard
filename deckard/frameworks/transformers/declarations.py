...
class GenericFlexibleTransformer(nn.Module if nn else object):
    ...
    """
    def __init__(self, model_name="bert-base-uncased", out_features=256, num_classes=2, pretrained=True):
        if nn is None or AutoModel is None:
            raise ImportError("GenericFlexibleTransformer requires 'torch' and 'transformers'.")
        super().__init__()
        ...
        self.hidden_dim = getattr(self.config, "hidden_size", 
                          getattr(self.config, "d_model", 
                          getattr(self.config, "embed_dim", None)))
        
        if self.hidden_dim is None:
            raise ValueError("Could not automatically determine backbone hidden_size dimension.")
    ...
    def forward(self, **kwargs):
        ...
        pooled_features = self._pool_hidden_states(outputs, attention_mask=attention_mask)
