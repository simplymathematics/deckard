...
def calculate_failure_rate(data):
    ...
    if "predict_time" in data.columns:
        ...
        survival_time = data.loc[:, "predict_time"] * (1 - data.loc[:, "accuracy"])
        ...
        surival_time = data.loc[:, "predict_proba_time"] * (1 - data.loc[:, "accuracy"])
    else:
    ...
    if "adv_fit_time" in data.columns:
        ...
        if "predict_time" in data.columns:
            ...
            adv_survival_time = data.loc[:, "predict_time"] * (1 - data.loc[:, "adv_accuracy"])
        elif "predict_proba_time" in data.columns:
            adv_failure_rate = (
                (1 - data.loc[:, "adv_accuracy"])
                * data.loc[:, "attack.attack_size"]
            ...
            adv_survival_time = data.loc[:, "predict_proba_time"] * (1 - data.loc[:, "adv_accuracy"])
        else:
            raise ValueError("predict_time or predict_proba_time not in data.columns")
