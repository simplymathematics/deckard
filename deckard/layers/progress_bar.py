...
def _count_completed_studies(observed_finished_counts: list, required_trials: list) -> int:
...
def progress_bar_main(
	...
	for stage in optuna_stages:
		...
		storage_requirements[storage]["required_trials"].extend([n_trials] * studies_count)

	expected_studies = sum(item["expected_studies"] for item in storage_requirements.values())
	expected_total_trials = sum(item["expected_trials"] for item in storage_requirements.values())
	...
	def _aggregate_progress() -> tuple:
		...
		for storage, req in storage_requirements.items():
			finished_counts, storage_earliest = _collect_storage_finished_counts(storage, end_states)
			...
			total_completed_studies += min(storage_completed_studies, req["expected_studies"])
			total_completed_trials += storage_completed_trials

			if storage_earliest is not None and (earliest is None or storage_earliest < earliest):
				earliest = storage_earliest

		return total_completed_studies, total_completed_trials, earliest

	initial_complete_studies, initial_complete_trials, db_start_time = _aggregate_progress()
