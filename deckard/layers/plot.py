...
def _resolve_plot_args_from_cfg(cfg: Any) -> Dict[str, Any]:
	...
	plot_block = cfg_dict.get("plot", {}) if isinstance(cfg_dict.get("plot"), dict) else {}
...
def _extract_backend(cfg: Any, data_file: str, experiment_cfg: Dict[str, Any], experiment_config: str) -> str:
	cfg_dict = _cfg_to_dict(cfg)
	plot_block = cfg_dict.get("plot", {}) if isinstance(cfg_dict.get("plot"), dict) else {}
	...
	if backend == "auto":
		...
		if experiment_config or experiment_cfg:
			return "yellowbrick"
		raise ValueError("Could not infer backend: provide plot.data_file or plot.experiment_config/experiment cfg")
	return backend
...
def plot_main(cfg: Any) -> dict:
	...
	backend = _extract_backend(cfg, data_file=data_file, experiment_cfg=extracted_experiment_cfg, experiment_config=experiment_config)
	...
	if backend == "yellowbrick" and not experiment_config:
		if not extracted_experiment_cfg:
			raise ValueError("yellowbrick backend requires plot.experiment_config or a Hydra experiment cfg")
	...
	if not plot_type and not plots and not plots_file:
		if backend == "yellowbrick":
			...
			raise ValueError("Provide one of plot.plot_type, plot.plots, or plot.plots_file.")
	if sum(bool(x) for x in [plot_type, plots, plots_file]) > 1:
		raise ValueError("Provide only one of plot.plot_type, plot.plots, or plot.plots_file.")

	if backend == "yellowbrick" and plots_file:
		raise ValueError("plot.plots_file is only supported for seaborn backend.")

	if backend == "yellowbrick":
		exp_cfg = extracted_experiment_cfg if extracted_experiment_cfg else _load_experiment_config(experiment_config)
		...
		from ..plot.yellowbrick_plots import YellowbrickConfigList, YellowbrickPlotConfig

		if plot_type:
			default_folder = Path(plot_folder) if plot_folder else Path.cwd()
			output_path = Path(plot_file) if plot_file else default_folder / f"{plot_type}.png"
		...
		if isinstance(plot_list, list) and len(plot_list) == 0:
			raise ValueError("--plots must contain at least one plot type for yellowbrick backend.")
...
if __name__ == "__main__":
	raise SystemExit("Run this layer via deckard entrypoint with Hydra, e.g. `deckard plot ...`")
