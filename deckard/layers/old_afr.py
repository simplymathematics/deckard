...
def plot_aft(
    ...
    **kwargs,
    ...
    plt.gcf().clear()
    # enforce numerical values
    df = df.apply(pd.to_numeric, errors="coerce")
    print(df.columns)
    input("Press Enter to continue...")
    aft.fit(df, duration_col=duration_col, event_col=event_col, **kwargs)
    ax = aft.plot()
...
if "__main__" == __name__:
    ...
    adv_failures = (1 - data.loc[:, "adv_accuracy"]) * data.loc[
        :,
        "attack.attack_size",
    ]
    ben_failures = (1 - data.loc[:, "accuracy"]) * data.loc[
        :,
        "attack.attack_size",
    ]
    data["adv_failures"] = adv_failures
    data["ben_failures"] = ben_failures
