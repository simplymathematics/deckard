...
class PromQuery:
    ...
    def query_prometheus(self):
        ...
        return abs(float(result[0]['values'][-1*int(self.total)][1])-float(result[0]['values'][-1][1]))

    def get_power(self):
        self.query = 'sum(increase((kepler_container_joules_total['+self.caluculate_minutes()+'])))'
...
def run_query(input_file, output_file):
    ...
    new_columns = ['train_power', 'predict_power', 'predict_proba_power', 'predict_log_proba_power', 'adv_fit_power',  'adv_predict_power']
    start_times = ['train_start_time', 'predict_start_time', 'predict_proba_start_time', 'predict_log_proba_start_time', 'adv_fit_start_time',  'adv_predict_start_time']
    end_times = ['train_end_time', 'predict_end_time', 'predict_proba_end_time', 'predict_log_proba_end_time', 'adv_fit_end_time',  'adv_predict_end_time']
