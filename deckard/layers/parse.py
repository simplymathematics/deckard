...
def parse(params: dict, stage: str = None, default: str = None):
    ...
    if default is not None:
        ...
        assert Path("params.yaml").exists(), "Path params.yaml does not exist"
    for entry in grid:
        ...
        logger.info(
            f"Successfully parsed the hydra config and saved it to {Path(entry['files']['reports'], entry['files']['path'], 'params.yaml')}",
        )
        logger.debug(yaml.dump(entry, indent=4))
...
def generate_paths_from_params(params: dict, stage=str) -> dict:
    ...
    if "data" in params:
        if "data_file" not in files:
            ...
            assert (
                data["filename"] is not None
                or data["path"] is not None
                or "data_file" in files
            ), "Either path or filename must be specified for the data or data_file must be specified in the files section"
            data["path"] = (
                params["data"]["path"]
                if params["data"]["path"] is not None
                else my_hash(data)
            )
            data["filetype"] = (
                params["data"]["filetype"]
                if params["data"]["filetype"] is not None
                else "npz"
            )
            data["filename"] = (
                params["data"]["filename"]
                if params["data"]["filename"] is not None
                else my_hash(data)
            )
            files["data_file"] = str(
                Path(
    ...
    if "model" in params:
        ...
        if "model_file" not in files:
            assert (
                model["filename"] is not None or model["path"] is not None
            ), "Either path or filename must be specified for the model or model_file must be specified in the files section"
            model["path"] = (
                params["model"]["path"]
            ...
            model["filetype"] = (
                params["model"]["filetype"]
                if params["model"]["filetype"] is not None
                else "pkl"
            )
            model["filename"] = (
                params["model"]["filename"]
                if params["model"]["filename"] is not None
                else my_hash(model)
            )
            files["model_file"] = str(
                Path(
                    model["path"],
    ...
    if "attack" in params:
        ...
        if "attack_file" not in files:
            assert (
                "path" in attack or "filename" in attack
            ), "Either path or filename must be specified for the attack"
            attack["path"] = (
                params["attack"]["path"]
                if params["attack"]["path"] is not None
                else my_hash(params)
            )
            attack["filetype"] = (
                params["attack"]["filetype"]
                if params["attack"]["filetype"] is not None
                else "pkl"
            )
            attack["filename"] = (
                params["attack"]["filename"]
                if params["attack"]["filename"] is not None
                else my_hash(attack)
            )
            files["attack_file"] = str(
                Path(
                    attack["path"],
                    attack["filename"] + "." + attack["filetype"],
                ).as_posix(),
            )
            del attack["path"]
            del attack["filename"]
    ...
    params["files"] = files
    assert (
        params["files"]["path"] is None
    ), f"Path is already specified as {params['files']['path']}"
