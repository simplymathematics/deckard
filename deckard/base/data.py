...
class Data(BaseHashable):
    ...
    """
    def __init__(self, dataset:str, target = False, time_series:bool = False, train_size:int = 100, random_state=0, shuffle:bool=True,  stratify=False, test_size:int = 100, path = None):
        """
    ...
    def __call__(self) -> None:
        ...
        if isinstance(self.dataset, str) and not Path(self.path, self.dataset).exists():
            if self.dataset.endswith(".csv") or self.dataset.endswith(".txt"):
                (X_train, y_train), (X_test, y_test) = self._parse_csv(self.dataset, self.target)
            elif self.dataset in SUPPORTED_DATASETS:
                (X_train, y_train), (X_test, y_test) , _, _ = load_dataset(self.dataset)
            else:
                raise ValueError("String dataset must be a csv file or a supported dataset.")
            self.X_train, self.y_train, self.X_test, self.y_test, minimum, maximum = self._sample_data(X_train, y_train, X_test, y_test)
            self.clip_values = (minimum, maximum)
        elif isinstance(self.dataset, Union[str, Path]) and Path(self.path, self.dataset).exists():
            ...
            raise ValueError(f"Dataset must be a csv file, a string, or a pickled Data object. Strings must be a supported dataset: {SUPPORTED_DATASETS}")
    ...
    def _sample_data(self, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray) -> None:
        ...
        if hasattr(self, 'test_size'):
            assert isinstance(self.test_size, int) or isinstance(self.test_size, NoneType), "test_size must be an integer"
            if self.test_size is not None and self.test_size <= len(X_test):
                stratify = y_test if self.stratify == True else None
                _, X_test, _, y_test = train_test_split(X_test, y_test, test_size = self.test_size, random_state=self.random_state, shuffle=self.shuffle, stratify=stratify)
                X_test = X_test[:self.test_size]
                y_test = y_test[:self.test_size]
                assert len(X_test) == self.test_size, "X_test has {} rows. test_size is {}".format(len(X_test), self.test_size)
            else:
                logger.warning("test_size is greater than the number of test samples. Setting test_size to {}".format(len(X_test)))
                pass
        else:
            pass
