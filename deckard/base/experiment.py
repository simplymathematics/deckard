...
class Experiment(
    ...
    def load(self) -> tuple:
        ...
        if (
            "data_file" in params["files"]
            and Path(params["files"]["data_file"]).exists()
        ):
        ...
        if len(params["model"]) > 0:
            ...
            model = {}
        if (
            "model_file" in params["files"]
            and Path(params["files"]["model_file"]).exists()
        ):
                ...
                if full_path.exists():
                    if full_path.suffix == ".json":
                        ...
                        raise NotImplementedError(
                            f"File type, {full_path.suffix}, not supported",
                        )
                else:
                    files[k] = v
    ...
    def save(
        ...
        if "ground_truth_file" in self.files and ground_truth is not None:
            ...
            assert Path(
                files["ground_truth"],
            ).exists(), f"Ground truth file {files['ground_truth']} does not exist."
        if "predictions_file" in self.files and predictions is not None:
            ...
            assert Path(
                files["predictions"],
            ).exists(), f"Predictions file {files['predictions']} does not exist."
        if "probabilities_file" in self.files and probabilities is not None:
            ...
            assert Path(
                files["probabilities"],
            ).exists(), f"Probabilities file {files['probabilities']} does not exist."
        ...
        if "score_dict_file" in self.files and score_dict is not None:
            ...
            assert Path(
                files["scores"],
            ).exists(), f"Scores file {files['scores']} does not exist."
        return files
    ...
    def run(
        ...
        if isinstance(model, Model) and len(self.model) > 0:
            ...
            hasattr(model, "fit")
            and "ground_truth_file" in files
            and isinstance(files["ground_truth_file"], str)
            ...
            hasattr(model, "fit")
            and "ground_truth_file" in files
            and not isinstance(files["ground_truth_file"], str)
            and len(self.model) > 0
        ):
        ...
        #######################################################################
        if (
            "predictions_file" in files
            and isinstance(files["predictions_file"], str)
            and len(self.model) > 0
        ):
            logger.info("Predicting")
            try:
                predictions, predict_time = self.predict(
                    loaded_data,
                    fitted_model,
                    art=art,
                )
            ...
            "predictions_file" in files
            and not isinstance(files["predictions_file"], str)
            and len(self.model) > 0
        ):
        ...
        #######################################################################
        if (
            "probabilities_file" in files
            and isinstance(files["probabilities_file"], str)
            and len(self.model) > 0
        ):
        ...
        elif "probabilities_file" in files and not isinstance(
            files["probabilities_file"],
            str,
        ):
        ...
        if (
            ...
            and isinstance(files["score_dict_file"], str)
            and len(self.model) > 0
        ):
            logger.info("Scoring")
            try:
        ...
        #######################################################################
        if (
            "attack_samples_file" in files
            and isinstance(files["attack_samples_file"], str)
            and len(self.attack) > 0
        ):
            attack = "!Attack:\n" + str(self._asdict())
        ...
        elif "attack_samples_file" in files and not isinstance(
            files["attack_samples_file"],
            str,
        ):
            ...
            rerun = False
            logger.info(
                "Attack samples already made. Checking if attack needs to be rerun.",
            )
                ...
                if isinstance(files[kwarg], str) and Path(files[kwarg]).exists():
                    if Path(files[kwarg]).suffix == ".json":
                        ...
                        attack_results.update(
                            {kwarg: yaml.load(files[kwarg], Loader=yaml.FullLoader)},
                        )
                    else:
                        raise NotImplementedError(
                            f"File type {Path(files[kwarg]).suffix} not implemented.",
                        )
                elif isinstance(files[kwarg], str) and kwarg not in [
                    "data_file",
                    "model_file",
                    "attack_file",
                ]:
                    rerun = True
                    break
                elif isinstance(files[kwarg], str) and kwarg in [
                    "data_file",
                    "model_file",
                    "attack_file",
                ]:
