...
class Scorer(
    ...
    def score_from_memory(self, ground_truth: pd.DataFrame, predictions: pd.DataFrame) -> None:
        """
        ...
        for name, scorer in zip(names, scorers):
            ...
            try:
                score = factory(obj_name, **scorer, y_pred=predictions, y_true=ground_truth)
    ...
    def save(
        ...
        score_file = Path(files.pop('path'), my_hash(self), files.pop('score_dict_file'))
