...
#         print(
#             f"Found {len(metr_paths)} metrics, {len(html_paths)} htmls, {len(pred_paths)} predictions, {len(param_paths)} params, {len(ground_truth_paths)} ground truths",
#         )
...
#         big["predictions"] = [
#             Path(big.index[i], "predictions.csv") for i in range(len(big))
#         ]
#         big["ground_truth"] = [
#             Path(big.index[i], "ground_truth.csv") for i in range(len(big))
#         ]
...
#         big["link"] = big["link"].apply(
#             lambda x: f'<a href="{x}">{str(x).split("/")[0]}</a>',
#         )
#         big["predictions"] = big["predictions"].apply(
#             lambda x: f'<a href="{x}">Predictions</a>',
#         )
#         big["ground_truth"] = big["ground_truth"].apply(
#             lambda x: f'<a href="{x}">Ground Truth</a>',
#         )
