...
class testVisualiser(unittest.TestCase):
    def setUp(self, exp_config = exp_config, data_config=data_config, model_config=model_config) -> None:
        ...
        data, model, _ = yaml.load("!Experiment:\n" + str(self.config), Loader=yaml.FullLoader).load()
        self.data = data.load()
        self.model = model.load(art = False)
        self.data.y_train = LabelBinarizer().fit(self.data.y_train).transform(self.data.y_train)
        self.data.y_test = LabelBinarizer().fit(self.data.y_train).transform(self.data.y_test)
        ...
        self.selection_viz = {
            "validation" : {"name" : "validation", "param_name" : "alpha", "param_range" : [0.001, 0.01, 0.1, 1.0, 10.0, 100.0],},
            "learning" : {"name" : "learning", "train_sizes" : [10, 50, 100, 200, 500, 640],},
            "cross_validation" : {"name" : "cross_validation", "param_name" : "alpha", "param_range" : [0.001, 0.01, 0.1, 1.0, 10.0, 100.0],},
