...
class testGenerator(unittest.TestCase):
    ...
    def test_generate_json(self):
        ...
        name = gen.list[0][0]
        params = gen.list[0][1]
        gen.generate_json(path = self.path, filename = self.filename, name = name, params = params)
        self.assertTrue(os.path.exists(os.path.join(self.path, self.filename + '.json')))
    
    def test_generate_yml(self):
        gen = Generator(self.model_yml, self.path, 'models')
        name = gen.list[0][0]
        params = gen.list[0][1]
        gen.generate_yml(path = self.path, filename = self.filename, name = name, params = params)
    ...
    def test_generate_directory_tree(self):
        ...
        for folder in folders:
            ...
            self.assertTrue(os.path.exists(os.path.join(self.path, 'models', folder, 'configs')))
