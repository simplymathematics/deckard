...
class testAttackExperiment(unittest.TestCase):
    ...
    def test_save_attack(self):
        ...
        for file in ['attack_params.json', 'data_params.json', 'experiment_params.json', 'model_params.json']:
    ...
    def test_run_attack_files(self):
        ...
        experiment = AttackExperiment(data =data, model = model, attack = attack)
        experiment(path = self.path)
        these = ['data_params.json', 'examples.json', 'time_dict.json', "predictions.json", "ground_truth.json"]
        for file in these:
