...
import collections
import numpy as np
from deckard.base.hashable import BaseHashable, my_hash, from_yaml, generate_line_search, generate_grid_search, generate_queue, sort_queue, from_dict
...
class testBaseHashable(unittest.TestCase):
    def setUp(self):
        ...
        with open(self.list, "w") as f:
            yaml.dump([
            {"files" :{"filetype" : "json", "path" : "foo", }},
            {"files" : {"filetype" : "csv", "path" : "foo", }},
            {"files" :{"filetype" : "xml", "path" : "foo", }},
        ], f)
    ...
    def test_save_yaml_load(self):
        ...
        with open(filename, "r") as f:
            self.assertEqual(yaml.load(f, Loader=yaml.FullLoader), self.hashable.to_dict())

    
    def test_save_yaml(self):
        filename = self.hashable.save_yaml(path = self.path)
        test_filename = Path(self.path) / Path(my_hash(self.hashable) + "." + self.filetype)
    ...
    def test_generate_line_search(self):
        exp_list = generate_line_search(self.hashable, "files.filetype", ["json", "csv", "xml"])
        self.assertEqual(exp_list[0].to_dict(), {"files" : {"filetype" : "json", "path" : "tmp", }})
        self.assertEqual(exp_list[1].to_dict(), {"files" : {"filetype" : "csv", "path" : "tmp", }})
        self.assertEqual(exp_list[2].to_dict(), {"files" : {"filetype" : "xml", "path" : "tmp", }})        
    
    def test_generate_grid_search(self):
        grid = {"files.filetype" : ["json", "csv", "xml"], "files.path" : ["foo", "bar", "baz"]}
        exp_list = generate_grid_search(self.hashable, grid)
        self.assertEqual(exp_list[0].to_dict(), {"files" : {"filetype" : "json", "path" : "foo", }})
        self.assertEqual(exp_list[1].to_dict(), {"files" : {"filetype" : "json", "path" : "bar", }})
        self.assertEqual(exp_list[2].to_dict(), {"files" : {"filetype" : "json", "path" : "baz", }})
        self.assertEqual(exp_list[3].to_dict(), {"files" : {"filetype" : "csv", "path" : "foo", }})
        self.assertEqual(exp_list[4].to_dict(), {"files" : {"filetype" : "csv", "path" : "bar", }})
        self.assertEqual(exp_list[5].to_dict(), {"files" : {"filetype" : "csv", "path" : "baz", }})
        self.assertEqual(exp_list[6].to_dict(), {"files" : {"filetype" : "xml", "path" : "foo", }})
        self.assertEqual(exp_list[7].to_dict(), {"files" : {"filetype" : "xml", "path" : "bar", }})
        self.assertEqual(exp_list[8].to_dict(), {"files" : {"filetype" : "xml", "path" : "baz", }})
    ...
    def test_from_dict(self):
        config = {"files" :{"filetype" : "json", "path" : "tmp", }}
