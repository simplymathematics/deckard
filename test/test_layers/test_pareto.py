...
class TestNormalizeDirection:
    def test_maximize(self):
        assert _normalize_direction("maximize") == "maximize"

    def test_max_alias(self):
        assert _normalize_direction("max") == "maximize"

    def test_minimize(self):
        assert _normalize_direction("minimize") == "minimize"

    def test_min_alias(self):
        assert _normalize_direction("min") == "minimize"

    def test_diff(self):
        assert _normalize_direction("diff") == "diff"

    def test_dotted_prefix_stripped(self):
        assert _normalize_direction("StudyDirection.maximize") == "maximize"
        assert _normalize_direction("StudyDirection.minimize") == "minimize"
...
class TestCompleteTrialsOnly:
    ...
    def test_filters_non_complete(self):
        ...
        assert len(result) == 2
        assert all(result["state"] == "COMPLETE")
...
class TestCoerceObjectiveColumnsNumeric:
    def test_already_numeric_unchanged(self):
        ...
        assert pd.api.types.is_numeric_dtype(result["accuracy"])

    def test_coerces_string_float_column(self):
        df = pd.DataFrame({"accuracy": ["0.9", "0.8"]})
        result = _coerce_objective_columns_numeric(df, ["accuracy"])
        assert result["accuracy"].iloc[0] == pytest.approx(0.9)
...
class TestInferDefaultOptimizers:
...
class TestObjectiveToColumn:
    ...
    def test_params_prefix_lookup(self):
        ...
        assert _objective_to_column("lr", 0, df, []) == "params_lr"
    ...
    def test_index_fallback_when_no_metric_names(self):
        df = pd.DataFrame({"values_0": [0.9]})
        assert _objective_to_column("anything", 0, df, []) == "values_0"
...
class TestInferDirectionsForObjectives:
    ...
    def test_uses_study_directions_when_lengths_match(self):
        ...
        assert result == ["minimize", "maximize"]
...
class TestParetoMain:
    ...
    def test_single_objective_maximize_saves_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "_selection_type" in result_df.columns
            assert result_df["_selection_type"].iloc[0] == "single_objective"
    ...
    def test_single_objective_minimize_selects_lowest(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result_df = pd.read_csv(output_path)
            assert result_df["value"].iloc[0] == pytest.approx(0.3)
    ...
    def test_multi_objective_produces_pareto_front(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert len(result_df) >= 1
            assert result_df["_selection_type"].iloc[0] == "pareto_front"
...
class TestParetoMainDefaultsAndValidation:
