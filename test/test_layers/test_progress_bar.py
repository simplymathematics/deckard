...
def test_count_studies_for_stage_conf_variants():
	assert progress_bar_module._count_studies_for_stage_conf({"matrix": {"a": [1, 2], "b": ["x", "y", "z"]}}) == 6
	assert progress_bar_module._count_studies_for_stage_conf({"foreach": ["a", "b", "c"]}) == 3
	assert progress_bar_module._count_studies_for_stage_conf({"cmd": "deckard optimize --multirun"}) == 1
...
def test_extract_and_resolve_stage_config_name(tmp_path):
	...
	_write_yaml(default_cfg, {"hydra": {"sweeper": {"storage": "sqlite:///db.sqlite3", "n_trials": 1}}})
	_write_yaml(stage_cfg, {"hydra": {"sweeper": {"storage": "sqlite:///db.sqlite3", "n_trials": 5}}})

	stage_conf = {"cmd": "deckard optimize --multirun --config-name inference-default"}

	assert progress_bar_module._extract_stage_config_name(stage_conf) == "inference-default"
	resolved = progress_bar_module._resolve_hydra_config_for_stage(stage_conf, str(default_cfg))
...
def test_get_hydra_sweeper_config_supports_flat_or_nested_hydra(tmp_path):
	...
	_write_yaml(nested_cfg, {"hydra": {"sweeper": {"storage": "sqlite:///nested.sqlite3", "n_trials": 11}}})
	_write_yaml(flat_cfg, {"sweeper": {"storage": "sqlite:///flat.sqlite3"}})

	assert progress_bar_module._get_hydra_sweeper_config(str(nested_cfg)) == ("sqlite:///nested.sqlite3", 11)
	# Defaults to 100 when n_trials is not set.
	assert progress_bar_module._get_hydra_sweeper_config(str(flat_cfg)) == ("sqlite:///flat.sqlite3", 100)
...
def test_collect_storage_finished_counts_reads_trials_and_earliest(tmp_path):
	...
	counts, earliest = progress_bar_module._collect_storage_finished_counts(db_url, {"COMPLETE", "FAILED", "PRUNED"})
...
def test_progress_bar_main_raises_for_unknown_stage(tmp_path):
	...
	_write_yaml(default_cfg, {"hydra": {"sweeper": {"storage": "sqlite:///x.sqlite3", "n_trials": 1}}})
