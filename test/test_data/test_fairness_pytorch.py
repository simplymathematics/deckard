...
def test_sensitive_extraction():
    ds = DummyDataset(20)
    config = FairlearnPytorchDataConfig(dataset_name="test.DummyDataset", sensitive_columns=["_sensitive"])
    ...
    all_sensitive = np.concatenate([np.array(config._sensitive_train), np.array(config._sensitive_test)])
