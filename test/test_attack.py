...
def test_attackconfig_get_benign_preds_shape():
    ...
    n, ben_pred_labels, X_subset, y_subset = config._get_benign_preds(DummyData(), DummyEstimator(), train=False)
    assert n == 5
...
def test_attackconfig_get_feature_vector_preds_shape():
    ...
    config = AttackConfig(attack_size=5)
    n, X_subset, y_subset, a_subset = config._get_feature_vector_preds(DummyData(), "a", train=False)
    assert n == 5
    assert X_subset.shape[0] == 5
    assert len(y_subset) == 5
