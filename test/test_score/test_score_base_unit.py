...
def test_scorer_config_post_init_dict_and_string_paths(monkeypatch):
    ...
    monkeypatch.setattr("deckard.score.base.resolve_class", lambda path: (lambda y_true, y_pred, **kwargs: 1.0))

    cfg = ScorerConfig(
        score_name="custom",
        score_function=OmegaConf.create({"_target_": "pkg.metric", "_args_": 3, "flag": True}),
        score_params=None,
    )
    ...
    cfg_str = ScorerConfig(score_name="acc", score_function="sklearn.metrics.accuracy_score")
...
def test_probability_validation_error_paths():
    scorer = ScorerConfig(score_name="roc_auc", score_function="sklearn.metrics.roc_auc_score", needs_proba=True)
    ...
    with pytest.raises(ValueError, match="numeric probabilities"):
        scorer._validate_probability_input([0, 1], np.array([["a"], ["b"]], dtype=object))
...
def test_normalize_predictions_branches_for_probability_and_labels():
    roc = ScorerConfig(score_name="roc_auc", score_function="sklearn.metrics.roc_auc_score", needs_proba=True)
    assert np.array_equal(
        roc._normalize_predictions_for_metric([0, 1], np.array([[0.2], [0.9]])),
        np.array([0.2, 0.9]),
    )
    assert np.array_equal(
        roc._normalize_predictions_for_metric([0, 1], np.array([[0.8, 0.2], [0.1, 0.9]])),
        np.array([0.2, 0.9]),
    )

    acc = ScorerConfig(score_name="accuracy", score_function="sklearn.metrics.accuracy_score")
    assert np.array_equal(
    ...
    assert np.array_equal(
        acc._normalize_predictions_for_metric([0, 1], np.array([[0.8, 0.2], [0.1, 0.9]])),
        np.array([0, 1]),
    )
...
def test_scorer_dict_init_iter_getitem_and_builders():
    ...
    assert isinstance(build_scorer({"score_name": "acc", "score_function": "sklearn.metrics.accuracy_score"}), ScorerConfig)
...
def test_scorer_dict_call_mode_and_probability_routing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scorer_dict, "save_scores", lambda scores, path: saved.update(scores))
    ...
    result_test = scorer_dict(mode="test", data=data, model=model, attack=None, score_file=score_file.as_posix())
    assert result_test["accuracy"] == 0.9
    assert "roc_auc" in result_test
    assert saved

    result_train = scorer_dict(mode="train", data=data, model=model, y_proba=np.array([0.2, 0.8]))
    assert "training_accuracy" in result_train
    assert "training_roc_auc" in result_train

    result_attack = scorer_dict(mode="attack", data=data, model=model, attack=attack, y_proba=np.array([0.7]))
    assert "attack_accuracy" in result_attack
    assert "attack_roc_auc" in result_attack

    result_val = scorer_dict(mode="val", data=data, model=model, y_proba=np.array([0.6, 0.6]))
    assert "accuracy" in result_val
    result_attack_val = scorer_dict(mode="attack-val", data=data, model=model, attack=attack_val)
    ...
    with pytest.raises(AssertionError, match="y_true must be provided if mode is None"):
...
def test_scorer_dict_attack_placeholder_and_missing_probability_context():
    scorer = ScorerDictConfig(
        scorers={
            "roc_auc": ScorerConfig(score_name="roc_auc", score_function="sklearn.metrics.roc_auc_score", needs_proba=True),
        },
    )
    ...
    with pytest.raises(ValueError, match="requires probabilities from predict_proba"):
        scorer(
            ...
            model=None,
            attack=SimpleNamespace(attack_size=1, attack_predictions=np.array([0]), _attack="resolved"),
            y_true=np.array([0]),
            y_pred=np.array([0]),
            extra="{attack}",
        )
