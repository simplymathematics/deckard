...
def _assert_wrapper_defenses(wrapper, expected_preprocessors, expected_postprocessors):
	assert _wrapper_defense_names(wrapper, "preprocessing_defences") == expected_preprocessors
	assert _wrapper_defense_names(wrapper, "postprocessing_defences") == expected_postprocessors
...
def test_experiment_config_accepts_legacy_single_defense_yaml():
	experiment = ExperimentConfig(
		...
		defense=str(ROOT / "examples/sklearn/config/defense/feature-squeezing.yaml"),
		files=FileConfig(),
		classifier=True,
...
@pytest.mark.parametrize(
	...
	[
		(
			[
				_example_defense("examples/sklearn/config/defense/feature-squeezing.yaml"),
				_example_defense(
		...
		(
			[
				_example_defense("examples/sklearn/config/defense/gaussian-noise.yaml"),
				_example_defense("examples/sklearn/config/defense/class-labels.yaml"),
			],
			[],
...
def test_defense_pipeline_config_hash_stable_after_apply():
    data, model, _ = _apply_defense({"defenses": [
        _example_defense("examples/sklearn/config/defense/feature-squeezing.yaml"),
    ]})
...
def test_model_config_with_defense_hash_stable_after_apply():
    data, model, _ = _apply_defense(str(ROOT / "examples/sklearn/config/defense/gaussian-noise.yaml"))
...
def test_defense_pipeline_scores_persist_and_reload():
    ...
    scores = {"defense_application_time": model.defense_application_time, "n_defenses": 2}
