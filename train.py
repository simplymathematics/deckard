...
from base.utils import checkpoint, load_data
import sys
import numpy as np
import yaml
import importlib
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--folder', type=str, help='Experiment folder to use', default = "data")
    parser.add_argument('-d', '--dataset', type=str, help='Data file to use', default = "data/data.pkl")
    ...
    if os.path.isdir(os.path.join(args.folder, 'best_train')):
    ...
    for model in models:
        ...
        checkpoint(filename = os.path.join("all_train", str(uuid.uuid4())), experiment = experiment, result_folder = args.folder)
        if score > best_score:
            best_score = score
