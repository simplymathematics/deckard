    ...
    def _choose_data(self, dataset:str='iris'):
        ...
        if os.path.isfile(dataset):
            if os.path.isfile(dataset) and dataset.endswith('.csv'):
                ...
                df.columns[-1] = 'target'
                data = {'data': df, 'target': df.pop('target')}
                assert df.shape > data['data'].shape
            elif os.path.isfile(dataset) and dataset.endswith('.json'):
                with open(dataset, 'r') as f:
                    data = json.load(f)
            elif os.path.isfile(dataset) and dataset.endswith('.npz'):
                data = np.load(dataset)
            elif os.path.isfile(dataset) and dataset.endswith('.pkl'):
